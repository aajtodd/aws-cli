...
pub async fn run(args: PresignArgs, ctx: &AppContext) -> Result<i32> {
    ...
    let config = match PresigningConfig::expires_in(Duration::from_secs(args.expires_in)) {
        Ok(c) => c,
        Err(e) => {
            termerrln!(ctx.term, "Invalid --expires-in: {e}")?;
            return Ok(exit_code::PARAM_VALIDATION_ERROR);
        }
    };
        ...
        .await
    {
        ...
        Err(ref e) => {
            ...
            termerrln!(
                ctx.term,
                "An error occurred ({code}) when calling the GetObject operation: {msg}"
            )?;
...
mod tests {
    ...
    async fn generates_url() {
        ...
        let rc = run(presign_args("s3://bucket/key", 3600), &ctx)
            .await
            .unwrap();
    ...
    async fn non_dns_compatible_bucket_falls_back_to_path_style() {
        ...
        let (ctx, term) = test_ctx_fixed_time();
        let rc = run(presign_args("s3://bucket.dots/key", 3600), &ctx)
            .await
            .unwrap();
    ...
    async fn custom_expires_in() {
        ...
        let (ctx, term) = test_ctx_fixed_time();
        let rc = run(presign_args("s3://bucket/key", 1000), &ctx)
            .await
            .unwrap();
    ...
    async fn s3_prefix_not_required() {
        ...
        let (ctx, term) = test_ctx_fixed_time();
        let rc = run(presign_args("bucket/key", 3600), &ctx).await.unwrap();
    ...
    async fn invalid_path_returns_252() {
        let (ctx, term) = test_ctx_fixed_time();
        let rc = run(presign_args("just-a-bucket", 3600), &ctx)
            .await
            .unwrap();
    ...
    async fn expires_in_too_long_returns_252() {
        ...
        let (ctx, term) = test_ctx_fixed_time();
        let rc = run(presign_args("s3://bucket/key", 604801), &ctx)
            .await
            .unwrap();

        assert_eq!(rc, 252);
        assert!(term.stderr_contents().contains("Invalid --expires-in"));
    }
}
